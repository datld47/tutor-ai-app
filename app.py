...
# DÁN ĐOẠN CODE NÀY VÀO THAY THẾ CHO KHỐI ĐÃ XÓA
...
###################################################################################################################
# Thêm class này vào app.py
# class CompilerDownloadDialog(tk.Toplevel):
#     def __init__(self, parent, missing_compilers, compiler_map):
#         super().__init__(parent)
#         self.title("Cài đặt Compiler còn thiếu")
...
#         tk.Label(self, text="Một số trình biên dịch cần thiết chưa được cài đặt.", font=("Arial", 11, "bold")).pack(pady=(10, 5))
#         tk.Label(self, text="Vui lòng chọn các mục cần tải và cài đặt:").pack(pady=5)

#         frame_checks = tk.Frame(self)
#         frame_checks.pack(pady=10, padx=20, fill='x')

#         for name in missing_compilers:
#             self.vars[name] = tk.BooleanVar(value=True) # Mặc định chọn sẵn
...
#         self.download_button = tk.Button(self, text="Tải và Cài đặt", command=self.start_download)
...
#         self.download_button.config(state="disabled") # Vô hiệu hóa nút khi đang tải
        
#         for name, var in self.vars.items():
#             if var.get(): # Nếu checkbox được chọn
#                 url = self.compiler_map[name]['url']
#                 # Gọi hàm tải và truyền callback để cập nhật status label
#                 download_and_extract_compiler(name, url, self.update_status)

#
# TÌM CLASS NÀY TRONG app.py VÀ THAY THẾ TOÀN BỘ NÓ
#
class CompilerDownloadDialog(tk.Toplevel):
    ...
    if values:
        ...
        for i,img in enumerate(exercise.get("image", [])): 
            if(img.get('link')!=''): 
                ...
                img_path_ = os.path.join(PATH_IMG, img['link'])
...
def main():
    ...
    ##################################GUI###############################################################
    window = tk.Tk()
    window.title(f"Tutor AI {APP_VERSION}")
    #window.title(f"Tutor AI")
    ...
    if login_app.result == 'ok':
        ...
        # Thêm lệnh "Hướng dẫn sử dụng" vào menu "Trợ giúp"
        ...
        btn_run_code.config(command=lambda: btn_run_code_click({'input': txt_input, 'output': txt_output, 'window': window}))
