...
#     #ucrt_path = os.path.abspath("../compiler/ucrt64/bin")
ucrt_path = get_path("../compiler/ucrt64/bin")
#else:   
    # ucrt_path = os.path.abspath("compiler/ucrt64/bin")
    #ucrt_path = os.path.abspath("compiler/ucrt64/bin")
os.environ["PATH"] = f"{ucrt_path};" + os.environ["PATH"]

# Đường dẫn đến thư mục bin của JDK b   ạn copy
#jdk_path = os.path.abspath(r"compiler/jdk-23/bin")
# if getattr(sys, 'frozen', False):    
#     jdk_path = os.path.abspath(r"../compiler/jdk-23/bin")
# else:   
#     jdk_path = os.path.abspath(r"compiler/jdk-23/bin")
jdk_path = get_path(r"compiler/jdk-23/bin")
os.environ["PATH"] = f"{jdk_path};" + os.environ["PATH"]

# Thêm đường dẫn đến Python interpreter mà bạn đã copy vào
# Đảm bảo đường dẫn này trỏ đến thư mục chứa python.exe (hoặc python trên Linux/macOS)
#python_path = os.path.abspath(r"compiler/python313") # <--- Đã cập nhật
# if getattr(sys, 'frozen', False):    
#     python_path = os.path.abspath(r"../compiler/python313") 
# else:   
#     python_path = os.path.abspath(r"compiler/python313") 
python_path = get_path(r"compiler/python313") 
os.environ["PATH"] = f"{python_path};" + os.environ["PATH"] # <--- Thêm vào PATH
...
def run_python(code): # <--- Hàm cho Python
    # Tạo thư mục tạm thời thủ công
    temp_dir = tempfile.mkdtemp(dir=DIR_TEMP)
    print(f"DEBUG: Thư mục tạm thời: {temp_dir}")
    ...
    try:
        ...
        # Lấy đường dẫn đầy đủ đến python.exe
        python_exe_full_path = os.path.join(python_path, "python.exe")

        if platform.system() == "Windows":
            # CÁCH KHẮC PHỤC MỚI: Truyền lệnh dưới dạng các đối số riêng biệt
            # Thay vì một chuỗi lệnh lớn cho shell
            # Lệnh sẽ là: start cmd /k "D:\path\to\python.exe" "D:\path\to\program.py" & pause
            
            # Sử dụng subprocess.list2cmdline để xử lý dấu ngoặc kép cho đường dẫn
            # Điều này đặc biệt hữu ích nếu đường dẫn có khoảng trắng.
            python_cmd_part = subprocess.list2cmdline([python_exe_full_path, python_file])
            
            # Xây dựng toàn bộ lệnh để truyền vào 'cmd /k'
            # Đảm bảo lệnh 'pause' nằm sau lệnh chạy Python
            full_cmd_to_run_in_new_cmd = f'{python_cmd_part} & pause'

            # Khi truyền 'full_cmd_to_run_in_new_cmd' cho 'start cmd /k',
            # chúng ta cần bọc nó trong một cặp dấu ngoặc kép duy nhất.
            # subprocess.Popen(["start", "cmd", "/k", "...", ...], shell=True) sẽ tự động làm điều này.
            
            subprocess.Popen(
                ["start", "cmd", "/k", full_cmd_to_run_in_new_cmd], # Truyền lệnh đã được chuẩn bị
                shell=True, # Vẫn cần shell=True cho lệnh 'start'
                cwd=temp_dir # Đặt thư mục làm việc hiện tại
            )
        elif platform.system() == "Linux":
            if os.path.exists(python_exe_full_path.replace("\\", "/")):
                 run_cmd = [python_exe_full_path.replace("\\", "/"), python_file]
            else:
                 run_cmd = ["python3", python_file] # Hoặc "python"
        ...
        return "Lỗi: Không tìm thấy trình thông dịch Python. Vui lòng kiểm tra cài đặt Python hoặc đường dẫn."
    except Exception as e:
