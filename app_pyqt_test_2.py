...
PATH_CHAT_VIEW_HTML=os.path.abspath(os.path.join(os.path.dirname(__file__), "static", "chat_view.html"))
...
class MyWeb(QWebEngineView):
    ...
    def __init__(self,path_html,chat_view_load_callback=None):
        super().__init__()
        self.chat_view_load_callback=chat_view_load_callback
        self.setPage(CustomWebEnginePage(self))
        self.loadFinished.connect(self.on_chat_view_loaded)
        local_url = QUrl.fromLocalFile(path_html)
        self.load(local_url)
        
    def on_chat_view_loaded(self, success):
        if success:
            print('load  myweb thành công')
            
        if self.chat_view_load_callback:
            self.chat_view_load_callback(success)

    def run_js(self,js_code,handle_js_result):
        self.page().runJavaScript(js_code,handle_js_result)
               

class ChatView(MyWeb):
    
    def run_js_test(self,message):
        
        def handle_js_result(result):
            pass
            
        js_code=f"test('{message}');"
        
        self.run_js(js_code,handle_js_result)
        
    def run_js_update_chat_container(self,message):
        ...
        js_code=f"update_chat_container({safe_message});"
...
# ******************************giao diện chat view************************************
    ...
    def build_right_panel(self):
        ...
        #------------------------------------------------------------------------------------#
        print("tạo web_view 2")
        
        self.web_view= ChatView(PATH_CHAT_VIEW_HTML,self.on_chat_view_loaded)
...
#-------------------------------------------------------------------------------------------------
    ...
    def on_editor_load_finished(self, ok):
        ...
        init_script = """
            if (typeof ClassicEditor !== 'undefined' && !window.editorInstance) { // Kiểm tra thêm instance
                ClassicEditor
                    .create( document.querySelector( '#editor' ), { language: 'vi' } )
                    .then( newEditor => {
                        window.editorInstance = newEditor; // Lưu instance vào biến toàn cục của trang
                ...
                console.log('CKEditor đã được khởi tạo trước đó hoặc không khả dụng.');
            }
        """
        try:
    ...
    def handle_gemini_response(self, response_text, was_retry):
        ...
        try:
            ...
            if not match:
                raise ValueError("Không tìm thấy đoạn JSON trong markdown.")
            ...
            data_=data['data']
            print(data_)
            #self.web_view.run_js_update_chat_container(data_)
            self.web_view.run_js_update_chat_container(data_)

        except:
            
            print('lõi json loads')
