...
def main():
    ...
    #if login.result=='ok':
    if (1):
        #window.attributes('-fullscreen', True) # Chế độ toàn
        #window.attributes('-topmost', True) # Thêm dòng này
        window.state('zoomed') # Hoặc window.attributes('-zoomed', True) nếu 'zoomed' không hoạt động trên hệ điều hành của bạn
        
        mssv= DICT_USER_INFO[0]['mssv']
        if(mssv=='0' or mssv=='1'):
            ACCOUNT_ROLE='ADMIN'
        ...
        window.grid_columnconfigure(0, weight=1,minsize=400)
        window.grid_columnconfigure(1, weight=1,minsize=500)
        ...
        #fr_left: left  hiển thị danh sách bài học ##############################################
        fr_left=tk.Frame(window)
        fr_left.grid(row=1,column=0,sticky='nswe',padx=5,pady=5)    
        
        #fr_center: center  hiển thị màn hình làm việc ##########################################
        fr_center=tk.Frame(window)
        fr_center.grid(row=1,column=1,sticky='nswe',pady=5)
        
        #fr_right hiển thị giao tiếp AI #########################################################
        fr_right=tk.Frame(window)
        fr_right.grid(row=1,column=2,sticky='nswe',padx=5,pady=5)
        ...
        # # fr_left: header hiển thị nút điều khiển #############################################
        # fr_left = tk.Frame(paned_window, bg='lightgray') 
        # paned_window.add(fr_left, weight=1) # Thêm weight ngay tại đây

        # # fr_center: center hiển thị màn hình làm việc ##########################################
        # fr_center = tk.Frame(paned_window, bg='darkgray') 
        # paned_window.add(fr_center, weight=2) # Thêm weight ngay tại đây

        # # fr_right hiển thị giao tiếp AI #########################################################
        # fr_right = tk.Frame(paned_window, bg='gray') 
        # paned_window.add(fr_right, weight=1) # Thêm weight ngay tại đây

        # # Cần cập nhật cửa sổ để có kích thước thực trước khi đặt sashpos
        # window.update_idletasks()
        # # total_width = window.winfo_width() # Không cần thiết nếu bạn đặt sashpos theo giá trị cố định
        
        # # Đặt vị trí của các đường phân cách (sashes) theo pixel
        # # Điều chỉnh các giá trị này để phù hợp với bố cục mặc định bạn muốn
        # left_width_initial = 400
        # center_width_initial = 500
        
        # paned_window.sashpos(0, left_width_initial) # Vị trí sash đầu tiên
        # paned_window.sashpos(1, left_width_initial + center_width_initial) # Vị trí sash thứ hai

        # end new---------------------------
        
        #fr_footer  hienr thị thông tin footter #################################################
        ...
        # fr_control.rowconfigure(0,weight=1)  
        ### label
        ...
        #các nút btn
        # Trong phần định nghĩa các nút trong fr_control:
        btn_exit = tk.Button(fr_control, text='Thoát', font=("Arial", 11), command=lambda: window_on_closing(window))
        btn_exit.grid(row=0, column=5, padx=5) # Đặt ở một cột khác, ví dụ column 5
        ### btn_send
        # btn_send=tk.Button(fr_control,text='send')
        # btn_send.grid(row=1,column=0)
        ### btn_help
        btn_help=tk.Button(fr_control,text='help')
        #btn_help.grid(row=1,column=1)
        ### btn_run_code
        # btn_run_code=tk.Button(fr_control,text='run code')
        # btn_run_code.grid(row=1,column=2)
        ### btn_clear_cache
        ...
        #btn_load_rule.grid(row=1,column=4)        
        ...
        txt_input = CodeEditor(
                ...
                font=("Consolas", 14),
                #highlighter="default",      # Thay đổi theme thành 'default' để có nền trắng
                blockcursor=True,
                #cursor="black",             # Thêm dòng này để đặt màu con trỏ là đen
                wrap="word")
        # txt_input=scrolledtext.ScrolledText(fr_input,font=("Arial", 14),wrap='word')
        ...
        ###txt_output
        #txt_output=scrolledtext.ScrolledText(fr_response)
